...
import com.neusoft.article.service.HotArticleService;
        ...
        if(responseResult.getCode().equals(200)){
            ...
            if(wmChannels != null && wmChannels.size() > 0){
                for (WmChannel wmChannel : wmChannels) {
                    List<HotArticleVo> hotArticleVos = hotArticleVoList.stream().filter(x -> x.getChannelId() == wmChannel.getId()).collect(Collectors.toList());
